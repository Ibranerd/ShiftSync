...
export default function ManagerFairnessPage() {
  ...
  const [scores, setScores] = useState<
    Array<{ userId: string; totalHours: number; premiumShiftCount: number; score: number }>
  >([])
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!userData.user) {
        setStatus("Please sign in to view fairness metrics.")
        return
      }
      ...
      const locationIds = (managerLocations ?? []).map((row: any) => row.location_id)
      ...
      const { data: staffLocationRows } = await supabase
        .from("user_locations")
        .select("user_id,location_id,revoked_at")
      ...
      if (activeStaffIds.size === 0) {
        setScores([])
        setUsers([])
        setStatus("No active staff certifications for managed locations.")
        return
      }

      const staffIds = Array.from(activeStaffIds)
      const { data: userRows } = await supabase
        .from("users")
        .select("id,full_name,desired_weekly_hours")
        .in("id", staffIds)
      ...
      const { data: assignments, error } = await supabase
        ...
        .neq("status", "dropped")
        .in("user_id", staffIds)
        .in("shifts.location_id", locationIds)
    ...
    <main className="flex min-h-[60vh] flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Distribution of hours and premium shifts across your managed locations.
        </p>
      </div>
